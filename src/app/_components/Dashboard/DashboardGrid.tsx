...
import { type DragEventHandler, use, useEffect, useRef, useState } from "react";
...
type GridItem = {
  ...
  shortName: string;
  tendency: number;
  marketData: { date: Date; value: number }[];
};
type DashboardGridProps = {
  onUpdateMarkets: (markets: string[]) => void;
...
export const DashboardGrid: React.FC<DashboardGridProps> = ({
  ...
  useEffect(() => {
    if (markets) {
      const temp: GridItem[] = markets.map((market) => market.market);
      setProcItems(temp);
      onUpdateMarkets(temp.map((market) => market.id));
    }
  }, [markets]);
  ...
  const dragStartEvent: DragEventHandler<HTMLDivElement> = (event) => {
    ...
    const gridY =
      grid.current.getBoundingClientRect().top - grid.current.scrollTop;
    ...
    const lastGap =
      lastSizeX > 1
        ? Math.round((gridWidth - lastSizeX * boxWidth) / (lastSizeX - 1))
        : 0;

    const boxCenterX = event.currentTarget.offsetLeft + boxWidth / 2.0;
    const boxCenterY =
      event.currentTarget.offsetTop - grid.current.scrollTop + boxHeight / 2.0;

    const boxIndexY = Math.floor(
      (boxCenterY - gridY + 16.0) / (boxHeight + 16.0),
    );
    const boxIndexX =
  ...
  const dragEvent: DragEventHandler<HTMLDivElement> = (event) => {
    ...
    if (
      event.clientX < gridX ||
      event.clientX >= gridX + gridWidth ||
      event.clientY < gridY ||
      event.clientY >= gridY + gridHeight
    ) {
      return;
    }
    ...
    const cursorIndexY = Math.floor(
      (event.clientY - gridY + 16.0) / (boxHeight + 16.0),
    );
    const cursorIndexX =
  ...
  return (
    <Group
      className="p-auto no-scrollbar overflow-y-scroll"
      ref={grid}
    ...
    >
      {procItems.map((item, index) => (
        <Container
          key={index}
          className="relative m-0! rounded-[25px] bg-[#9BA4B5] p-0!"
        >
          <Group
            className="rounded-[25px] border-2! border-transparent! bg-[#F1F6F980] p-[20px] hover:cursor-pointer! hover:border-[#394867]!"
            align="center"
            onClick={() => onOpenMarket(item.id)}
          >
            <Stack align="center" className="w-[150px] text-4xl">
              <p>{item.shortName}</p>
              <Group
                className={
                  "rounded-[8px] border-2! border-[#212A3E] bg-linear-to-b p-[10px] text-3xl text-white " +
                  (item.tendency < 0
          ...
          <Image
            ...
            onDragEnd={() =>
              updateDashboard.mutate(procItems.map((market) => market.id))
            }
            className="absolute top-[10px] left-[10px] h-[24px]! w-[24px]! object-none! hover:cursor-grab active:cursor-grabbing"
          />
        </Container>
      ))}
    </Group>
